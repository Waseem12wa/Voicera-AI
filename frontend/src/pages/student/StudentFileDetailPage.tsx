...
import { getStudentFileDetails, downloadFileSummary } from '../../services/studentService'
...
const StudentFileDetailPage: React.FC = () => {
	...
	const { data: fileDetails, isLoading, error, refetch } = useQuery({
		queryKey: ['studentFileDetails', fileId],
		queryFn: () => getStudentFileDetails(fileId!),
		enabled: !!fileId,
		retry: 3
	})
	...
	// Auto-generate additional info when file loads
	useEffect(() => {
		if (fileDetails && !additionalInfo) {
			setIsGeneratingInfo(true)
			// Simulate additional info generation for students
			setTimeout(() => {
						...
						{isGeneratingInfo ? (
							...
							<Box sx={{ 
								...
								{additionalInfo ? (
									...
									<Typography variant="body1" sx={{ lineHeight: 1.6 }}>
										Additional information will be generated automatically.
									</Typography>
								)}
							</Box>
						)}
...
export default StudentFileDetailPage
